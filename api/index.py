...
if not all([SUPABASE_URL, SUPABASE_KEY, SUPABASE_SERVICE_ROLE_KEY, SUPABASE_JWT_SECRET]):
    raise RuntimeError(
        "Supabase environment variables not set. Ensure SUPABASE_URL, SUPABASE_KEY, "
        "SUPABASE_SERVICE_ROLE_KEY and SUPABASE_JWT_SECRET are defined."
    )
...
def get_progress(current_user: User = Depends(get_current_user)):
    try:
        ...
        responses_res = (
            supabase_client.table("exercise_responses")
        ...
        trends: Dict[str, List[AssessmentResult]] = {}
        for sub in assessments_res.data:
            name = sub["assessments"]["name"]
            trends.setdefault(name, []).append(AssessmentResult(**sub))
...
@app.get("/supporter/users/{user_id}")
...
def get_assets(current_user: User = Depends(get_current_user)):
    try:
        ...
        for asset in assets_res.data:
            url_res = supabase_client.storage.from_("assets").create_signed_url(
                asset["storage_path"], 3600
            )
